...
func (h *Index[T, U]) Insert(txnID txns.TxnID, key U, rid RID) (err error) {
	...
	bucketPageSt := getBucketPage[U](bucketPage.GetData())

	if bucketPageSt.entriesCnt < root.maxKeysInBucketCount {
		...
		bucketPage.SetData(newData)
		bucketPage.SetDirtiness(true)
